...
| [`touch_speed.py`](touch_speed.py) | **(v3)** 反応速度の実験。高速ポーリング＋エッジ（押下/解放）時のみシリアル送信。display はバックグラウンド割り込み駆動に任せる。起動時に `loop_hz` を計測。 |
| [`microbit_plot.html`](microbit_plot.html) | PC 側のライブグラフツール。Web Serial API でシリアルを受信し、数値をリアルタイムに折れ線グラフ表示する。 |
| [`docs/conductive_pla.md`](docs/conductive_pla.md) | 設計メモ：導電性PLA（札サイズで 10〜30kΩ）を要素技術として 3 状態識別に活かす検討。 |
...
## v3: 反応速度の実験（高速ポーリング＋エッジのみシリアル）

`touch_speed.py` は内蔵容量タッチがどこまで速く反応できるかを測る実験コード。

- メインループは `is_touched()` をできるだけ高速にポーリング（実測 約20kHz / 約51µs/回）
- LED 表示は CODAL のタイマー割り込み（バックグラウンド）に任せ、状態変化時のみ 1 画素を書く
- シリアル送信は押下/解放のエッジ時のみ（`P 時刻ms ポーリング率` / `R ...`）
- 起動時に 1 秒間の素のループ回数 `loop_hz` を出力

### 計測でわかったこと

メインループは律速ではない（MicroPython でも ~20kHz）。一方、押下→解放の継続時間が
**40ms 刻みに量子化**して観測され、内蔵 `is_touched()` の容量サンプリングは
**約 40ms 周期（≒25Hz）でしか更新されない**ことが判明した。つまりどれだけ速くポーリングしても
**検出の最悪遅延は ~40ms が天井**になる。競技かるたの払い手（数ms〜10ms）には不足のため、
次は自作アクティブ検出（ピンを自分で充放電しサンプリング周期を支配する方式）へ移行する。
